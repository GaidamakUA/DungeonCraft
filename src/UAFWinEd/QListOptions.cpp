...
CQListOptions::CQListOptions(QUESTION_LIST_DATA& event, CWnd* pParent /*=NULL*/)
	: CDialog(CQListOptions::IDD, pParent)
{
	//{{AFX_DATA_INIT(CQListOptions)
	m_Label1 = _T("");
	m_Label2 = _T("");
	m_Label3 = _T("");
	m_Label4 = _T("");
...
void CQListOptions::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CQListOptions)
	...
	DDX_Text(pDX, IDC_TEXT, m_Title);
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CQListOptions, CDialog)
	//{{AFX_MSG_MAP(CQListOptions)
	ON_BN_CLICKED(IDC_SEEART, OnSeeart)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CQListOptions message handlers

BOOL CQListOptions::OnInitDialog() 
{
	CDialog::OnInitDialog();
	
   m_Title  = m_event.buttons.title;
...
void CQListOptions::OnOK() 
{
   UpdateData(TRUE);

   m_event.buttons.title = m_Title;
      ...
      )
   {
     ...
     //if ((labelPostChainOptionsType) m_AfterChain1.GetCurSel() ==; 
   };
...
void CQListOptions::GetData(QUESTION_LIST_DATA& event)
{
	event = m_event;
}
