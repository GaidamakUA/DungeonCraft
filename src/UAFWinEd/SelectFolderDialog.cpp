...
/*  
  {
    CRect rect;
  //pFD->GetDlgItem(cmb1)->GetWindowRect(rect);
  pFD->GetDlgItem(cmb2)->GetWindowRect(rect);
  //pFD->GetDlgItem(cmb3)->GetWindowRect(rect);
  //pFD->GetDlgItem(cmb4)->GetWindowRect(rect);
  pFD->ScreenToClient(rect);
    rect.left += 50;
  pFD->GetDlgItem(cmb2)->SetWindowPos(0,rect.left, rect.top, 0,0, SWP_NOZORDER|SWP_NOSIZE );
  };
...
//    CString msg;
//    CRect rect;
//    pFD->GetWindowRect(&rect);
//    msg.Format(" %d %d %d %d", rect.top, rect.bottom, rect.left, rect.right);
//    MsgBoxInfo(msg, "dialogRec");
...
//  };
  ...
  rectOKPB.left = rectCancelPB.left - rectOKPB.Width() - 4;
  rectOKPB.top  = rectCancelPB.top;


	pFD->GetDlgItem(IDOK)->SetWindowPos(0,rectOKPB.left, rectOKPB.top, 0,0, SWP_NOZORDER | SWP_NOSIZE);
  ...
  pFD->GetDlgItem(lst1)->GetWindowRect(rectList2LC);
	...
	pFD->GetDlgItem(stc4)->SetWindowText("Design:");


	CRect rectCurrFolderEB;
	pFD->GetDlgItem(edt1)->GetWindowRect(rectCurrFolderEB);
	pFD->ScreenToClient(rectCurrFolderEB);
	rectCurrFolderEB.left = 8;//rectstc3ST.left;
	//rectCurrFolderEB.right -= 70;
  rectCurrFolderEB.right = rectCurrFolderEB.left + 200;
	rectCurrFolderEB.top += 30;
	rectCurrFolderEB.bottom += 30;

	m_pEdit = new CEdit();
	m_pEdit->Create(WS_TABSTOP | WS_VISIBLE | WS_CHILD | ES_AUTOHSCROLL, 
					rectCurrFolderEB, pFD, IDEB_FOLDER_DIALOG_SEL_FOLDER);
  //m_pEdit->SetWindowPos(0,0,rectCurrFolderEB.right,rectCurrFolderEB.top,rectCurrFolderEB.bottom, SWP_NOZORDER);
...
void CSelectFolderDialog::OnLBSelChangedNotify( UINT nIDBox, UINT iCurSel, UINT nCode)
{
};
