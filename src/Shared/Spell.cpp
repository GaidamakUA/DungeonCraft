...
bool SPELL_DATA::CompileEndScript(void)
{
  if (SpellEndSource.IsEmpty()) return false;
  if (SpellEndBinary.IsEmpty())
  {
    SpellEndBinary = CompileScript(SpellEndSource, "End");
...
CString SPELL_DATA::CompileScript(const CString& script, const char *namePostfix)
{
...
//
//*****************************************************************************
void SPELL_DATA::ExecuteSpellEndScript(CHARACTER *pAttacker, CHARACTER *pTarget)
{ 
  if (!SpellEndSource.IsEmpty())
  {
    if (CompileEndScript())
...
//
//*****************************************************************************
BOOL ACTIVE_SPELL_LIST::Remove(int index, SPELL_DATA *pSpell)
{
//  CSingleLock sLock(&m_CS, TRUE);
  POSITION pos=NULL;
  if ((pos = m_spellList.FindKeyPos(index)) != NULL)
