...
//  DECLARE_SERIAL( ITEM_LIST )
  ...
  ~ITEM_LIST() { Clear(); }
  void Clear() { if (GetCount()) itemsModified = true; m_items.RemoveAll(); rdyItems.Clear(); }  
  ...
  miscErrorType CanReady(int item, CHARACTER *pChar);
  BOOL CanReady(itemReadiedType type, CHARACTER *pChar, const ITEM *pItem) const;
  BOOL CanUnReady(int item);
  BOOL UnReady(int item);
  //BOOL ClearReadyByType(itemLocationType type);
  //BOOL Ready(int item, BYTE ItemMask, CHARACTER *pChar);
  BOOL Ready(int item, CHARACTER *pChar);
  //BOOL Ready(const ITEM_ID& itemID, itemReadiedType irt, BYTE itemMask, CHARACTER *pChar);
  BOOL Ready(const ITEM_ID& itemID, itemReadiedType irt, CHARACTER *pChar);
  int  GetReadiedCount(itemReadiedType type) const;
  ...
  //int  GetReady(itemLocationType type) const;
  int  GetReadiedItem(itemReadiedType type, int pos) const;
...
#ifdef UAFEDITOR
  void AddUsableBaseclass(int usableFlags, int baseclassFlag, const CString& name);
#endif
