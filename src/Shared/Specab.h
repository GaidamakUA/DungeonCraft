...
struct SCRIPT_CONTEXT
  ...
  LPCSTR           sourceType;        // eg: "CHARACTER" or "SPELL"
  LPCSTR           sourceName;        // eg: "George" or "Heal"
  LPCSTR           scriptName;
...
*/
  ...
  void MsgBoxError(const char *context); // Eg: MsgBoxError("Spell");

  void SetSA_Source_Type          (LPCSTR source_Type);
  ...
  void Clear(void);
  const SPECIAL_ABILITIES *GetItemSAContext(void) const;
  SPECIAL_ABILITIES *GetAttackerSAContext(void);
  SPECIAL_ABILITIES *GetTargetSAContext(void);
  SPECIAL_ABILITIES *GetCharacterSAContext(void);
#ifdef UAFEngine
  SPECIAL_ABILITIES *GetCombatantSAContext(void);
#endif
  const SPECIAL_ABILITIES *GetClassSAContext(void);
  const SPECIAL_ABILITIES *GetBaseclassSAContext(void);
  const SPECIAL_ABILITIES *GetSpellSAContext(void) const;
  const SPECIAL_ABILITIES *GetSpellgroupSAContext(void);
  const SPECIAL_ABILITIES *GetMonstertypeSAContext(void) const;
  const SPECIAL_ABILITIES *GetRaceSAContext(void);
  const SPECIAL_ABILITIES *GetAbilitySAContext(void);
  const SPECIAL_ABILITIES *GetTraitSAContext(void);
  
  const ITEM_DATA       *GetItemContext(void) const;
  int                    GetItemContextKey(void){return itemKey;};
  CString                GetAttackerContext(void);
  CString                GetTargetContext(void);
  CString                GetCharacterContext(void);
  CString                GetCombatantContext(void);
  COMBATANT             *GetAttackerCombatant(void);
  COMBATANT             *GetTargetCombatant(void);
  CHARACTER             *GetCharacter(void);
  CHARACTER             *GetCreatedCharacterContext(void);
  COMBATANT             *GetCombatant(void);
  const CLASS_DATA      *GetClassContext(void);
  const BASE_CLASS_DATA *GetBaseclassContext(void);
  const RACE_DATA       *GetRaceContext(void);
  const SPELL_DATA      *GetSpellContext(void) const;
  SPELLGROUP_DATA       *GetSpellgroupContext(void);
  const MONSTER_DATA    *GetMonstertypeContext(void) const;
  ABILITY_DATA          *GetAbilityContext(void);
