...
void COMBATANT::EnterGuardingState(void)
{
  ...
  RunCombatantScripts(GUARDING_GUARD, ScriptCallback_RunAllScripts, NULL);
}
...
void COMBATANT::InstantSpellActivate(const SPELL_ID& spellID, 
                                     int target, 
...
void /* PRS July 2009 BOOL */ COMBATANT::UpdateCombatant()
{
  ...
  */
  ...
  if (onAuto)
  {
    ...
    {
      if (combatData.IsFreeAttacker()) return;  // No need to Think
      if (!Think()) 
...
bool COMBAT_DATA::CanReady(itemReadiedType type,
                           CHARACTER       *pChar, 
                           const ITEM      *pItem,
...
bool COMBAT_DATA::CanReady(const ITEM *pCharItem, const ITEM_DATA *pItem, CHARACTER *pChar)
{
  if (pCharItem->itemID.IsMoney())                return false;
  if (pCharItem->qty <= 0)                        return false;
  if (pItem == NULL)                              return false;
  if (pItem->Hands_to_Use > 2)                    return false;
  if (!pItem->IsUsableByClass(pChar->GetClass())) return false;
  if (!itemUsesRdySlot(pItem))                    return false;
...
void COMBAT_DATA::ListWeapons(COMBAT_SUMMARY_COMBATANT *pCSC)
{
  ...
  while (pos != NULL)
  {
    ...
    if ((pItem = itemData.GetItem(pCharItem->itemID)) != NULL)
    {
      ...
      {
        if (CanReady(pCharItem, pItem, pCharacter))
        {
...
void COMBAT_DATA::ListAttacks(COMBAT_SUMMARY_COMBATANT *pCSC)
{
  int atkIdx, numAtk;
...
void COMBAT_DATA::ListCombatants(COMBAT_SUMMARY *pCombatSummary, COMBATANT *pActiveCombatant)
{
  int i, j, k;
                                      ...
                                      int              distance22)
{
  ...
  pWeapon = combatSummary.PeekCombatant(activeCombatantIndex)->PeekWeapon(weaponIndex);
...
*/
  ...
  else
  {
    ...
    else
    {
      ...
      if (RunMeleeWeaponFilter(pCombatSummary, &csa) != 0) return;
    };
  };
                                      ...
                                      int              distance22)
{
  ...
  pAttack = combatSummary.PeekCombatant(activeCombatantIndex)->PeekAttack(attackIndex);
...
void COMBAT_DATA::ListActions(COMBAT_SUMMARY *pCombatSummary,int activeCombatant)
{
  ...
  for (curTarget=0; curTarget<numCombatants; curTarget++)
  {
    ...
    ListActionsForTarget(pCombatSummary, activeCombatant, curTarget, friendly);
  };
}
...
void DumpWeapon(FILE *f, int weaponOrdinal, int activeCombatant, int targetOrdinal)
{
//  int range2;
  const WEAPON_SUMMARY *pWeapon;
  ...
  pTarget = combatData.GetCombatant(combatSummary.PeekCombatant(targetOrdinal-1)->index);
  ...
  {
    pWeapon = combatSummary.PeekCombatant(activeCombatant)->PeekWeapon(weaponOrdinal-1);
...
void DumpAmmo(FILE *f, int ammoOrdinal, int activeCombatant, int targetOrdinal)
{
//  int range2;
  const AMMO_SUMMARY *pAmmo;
  const COMBATANT *pTarget;
  ...
  {
    pAmmo = combatSummary.PeekCombatant(activeCombatant)->PeekAmmo(ammoOrdinal-1);
    if (pAmmo->pAmmo == NULL) 
    {
      ammoName = "No Ammo";
    }
...
void DumpAttack(FILE *f, int attackOrdinal, int activeCombatant, int targetOrdinal)
{
  const COMBATANT *pTarget;
  const ATTACK_SUMMARY *pAttack;
  pTarget = combatData.GetCombatant(combatSummary.PeekCombatant(targetOrdinal-1)->index);
  ...
  pAttack = combatSummary.PeekCombatant(activeCombatant)->PeekAttack(attackOrdinal-1);
...
void DumpCombatant(FILE *f, int ordinal)
{
  const COMBATANT *pCombatant;
  const COMBAT_SUMMARY_COMBATANT *pCSC;
  const char *asciiState;
  pCSC = combatSummary.PeekCombatant(ordinal-1);
...
void DumpAction(FILE *f, int i, int action)
{
  const COMBAT_SUMMARY_ACTION *pAction;
  const COMBAT_SUMMARY_COMBATANT *pCOMBATANT;
  int   targetOrdinal;
  pAction = combatSummary.PeekAction(action);
  targetOrdinal = pAction->targetOrd;
  pCOMBATANT = combatSummary.PeekCombatant(targetOrdinal-1);
  fprintf(f, "Action number %d  ;  Action Type = %s\n", i, typeName[pAction->actionType]);
  fprintf(f, "  damage = %d\n", pAction->damage);
  fprintf(f, "  Target = combatant #%d  ", combatSummary.PeekCombatant(pAction->targetOrd-1)->index);
  DumpCombatant(f, pAction->targetOrd);
  ...
  if (pAction->weaponOrd == 0)
  {
    if (pAction->attackOrd == 0)
    {
      ...
      {
        NotImplemented(0x221fd, false);
      };
    }
    else
    {
      DumpAttack(f, pAction->attackOrd, 0, pAction->targetOrd);
    };
  }
  else
  {
    DumpWeapon(f, pAction->weaponOrd, 0, pAction->targetOrd);
    DumpAmmo(f, pAction->ammoOrd, 0, pAction->targetOrd);
  };
}
#endif

void DumpActions(CArray<int,int> *index)
{
#ifdef dumpactions
  int action;
  CString fname;
  COMBATANT *pCombatant;
  pCombatant = combatData.GetCombatant(combatSummary.PeekCombatant(0)->index);
  fname.Format("%sCombatActions%04d.txt",rte.DesignDir(),combatSummary.m_thinkCount);
  FILE *f;
  f = fopen(fname, "w");
  if (f == NULL)
  {
  ...
  fprintf(f, "%d Possible Actions for combatant %d  ***[%s]*****************\n",
                 ...
                 combatSummary.PeekCombatant(0)->index,
                 (LPCSTR)pCombatant->GetName());
  fprintf(f, " x = %d\n", pCombatant->x);
  fprintf(f, " y = %d\n", pCombatant->y);
  for (action=0; action<index->GetSize(); action++)
  {
    DumpAction(f, action, (*index)[action]);
  };
  fclose(f);
#endif
}
...
{//8a0c
  ...
  useScriptedAI = false;
  m_pCharacter->ReadyWeapon(NO_READY_ITEM);
  m_pCharacter->ReadyShield(NO_READY_ITEM);
  {
    ...
    if (!kernelError)
    {
      ...
      combatData.ListCombatants(&combatSummary, this);
      ...
      combatData.ListAttacks(combatSummary.GetCombatant(0)); // List attacks for attacker only.
      combatData.ListReachableCells(&combatSummary, combatSummary.GetCombatant(0)); // Number of steps to each cell.
      combatData.ListActions(&combatSummary, 0);
    ...
    case AT_SpellCaster:
      {
        ...
        m_pCharacter->ReadyWeapon(itemIdx);
    ...
    case AT_MeleeWeapon:
      {
        ...
        m_pCharacter->ReadyWeapon(bestWeaponIdx);
        AddTarget(dude, false);
        ReadyBestArmor();
        State(ICS_Attacking);
      };
      return TRUE;
    case AT_Unknown:
      NotImplemented(0xa88, false);
      break;
    default:
      NotImplemented(0x55ab, false);
      break;
    };
  };
  ...
  { // *********************************************************** SHIELD
    ...
    {
      m_pCharacter->ReadyShield(pcsc->PeekShield(pcsc->shieldToReady-1)->index);
    };
  };
  if (!useScriptedAI)
  { // *********************************************************** WEAPON
            ...
            if (m_pCharacter->myItems.GetCharges(itemIdx) > 0)
            {
              ...
              if (spellID.IsValidSpell())
              {
                ...
                if (pos != NULL)
                {
                  ...
                  //NotImplemented(0x7c5da86, false);
                  // We may have to do some fiddling here.
                  // Setting the caster baseclass and spell school to
                  // illegal values for now......
                  {
  ...
  case ICS_Moving:    
    if (hPath < 0) 
    {
      ...
      if (  (moveX>=0) && (moveY>=0) && coordsOnMap(moveX, moveY, width, height))
      {
        ...
        FindPathTo(moveX, moveY, moveX, moveY, !zeroMoveAttackOK, TRUE);
      }
      else 
      {
                               ...
                               int wpn)
{
  ...
  // Removed 20130215 at the direction of Manikus  PRS
  backstabSkillID = Skill_BackstabMultiplier;
  if (!GetAdjSkillValue(backstabSkillID,pAttacker->m_pCharacter,NULL,NULL,&m_backstabMultiplier))
  {
    m_isBackStab = FALSE;
  }
  else
...
*/
  ...
  };
  {
    ...
    {
      itemName = scriptContext.GetItemContext()->UniqueName();
    }
    else
    {
      itemName = "Bogus Item";
    ...
    if (itemID.IsValidItem())
    {
      ...
      if (pItem != NULL)
      {
        ...
        {
          result = scriptContext.GetSpellContext()->RunSpellScripts
                      (DOES_ATTACK_SUCCEED,
                       ScriptCallback_RunAllScripts,
                       "YN");
        if (!result.IsEmpty()) finalResult = result;
...
int COMBATANT::makeAttack(int targ, int extraAttacksAvailable, int *pDeathIndex)
{
  ...
  { 
    ...
    SPELL_ID attackSpellID;
    if (wpn != NO_READY_ITEM)
    {  
      const SPELL_DATA *pSpell;
      pSpell = spellData.PeekSpell(pWeapon->SpellID());
      attackSpellID = pWeapon->SpellID();
      if (attackSpellID.IsValidSpell())
      {
        //NotImplemented(0x551c, false);
      }
      else
      {
        CString msg;
    ...
    InstantSpellActivate(attackSpellID, targ, &toHitComputation);
    if (attackSpellID.IsEmpty() || (toHitComputation.BeginSpellScriptFailure() == 0))
    {
                                ...
                                toHitComputation.BackstabMultiplier());
      {
        ...
        if (!itemID.IsNoItem())
        {
          ...
          if (pWeapon->IsUsable() && (pWeapon->Wpn_Type == SpellCaster))
          {
            ...
            InstantSpellActivate(pWeapon->spellID, targ, &toHitComputation);
          };
          if (pWeapon->IsUsable() && (pWeapon->Wpn_Type == SpellLikeAbility))
          {
            //InstantSpellActivate(pWeapon->m_gsID, targ);
            //NotImplemented(0x553, false);
            InstantSpellActivate(pWeapon->spellID, targ, &toHitComputation);
          };
        };
      };
...
#ifdef TraceFreeAttacks
    {
      //if (virtualGameTime == 0x128d9c)
      //{
      //  ASSERT(1);
      //  WriteDebugString("Combatant.cpp line 5019\n");
      //};
    ...
    if (wpn != NO_READY_ITEM)
    {
      ...
      // 20110519 PRS  if (!myItems.HaveItem(*(GLOBAL_ITEM_ID*)&wpn)) // if deleted because of zero quantity
      {
        ...
        if (pItem!=NULL) 
        {
          ...
          m_pCharacter->specAbs.RunScripts(ON_USE_LAST_WEAPON, ScriptCallback_RunAllScripts, NULL, GetName(), " Used last weapon");
          ClearCharContext();
  ...
  if (!continueAttack)
  {
    ...
    if (dude != NO_DUDE)
    {
      if (qcomb.DelayedX() >= 0)
      {
        ...
        pDude->x = qcomb.DelayedX();
        pDude->y = qcomb.DelayedY();
      };
    };
    return;
  };
...
// PURPOSE:  
//*****************************************************************************
BOOL COMBATANT::IsDone(bool freeAttack)
{ 
  //QueuedCombatantData &qcomb = GetQueuedCombatants();
  //if (   (qcomb.Top() == self)
  //    && (!qcomb.ChangeStats()))
  ...
  if (m_isCombatReady < 0)
  {
    ...
    scriptContext.SetCharacterContext(this->m_pCharacter);
    scriptContext.SetCombatantContext(this);
    if (!m_pCharacter->specAbs.RunScripts(IS_COMBAT_READY, ScriptCallback_LookForChar, "N", GetName(), "").IsEmpty())
    {
      m_isCombatReady = 0;
    }
    else
    {
      if (!combatantSA.RunScripts(IS_COMBAT_READY, ScriptCallback_LookForChar, "N", GetName(), "").IsEmpty())
      {
        m_isCombatReady = 0;
      }
    };
...
{
  return IsDone(false) ? "" : m_specialActionName;
}
...
{
  return IsDone(false) ? -1 : m_specialActionIndex;
}
...
BOOL COMBATANT::CanMove(BOOL allowZeroMove)
{
  ...
  if (m_iMovement >= GetAdjMaxMovement())
  {
    return FALSE;
  };
  return TRUE;
...
{  
  ...
  hPath = pathMgr.GetPath(x, y, destLeft, destTop, destRight, destBottom, CheckOccupants, this);
  return (hPath >= 0);
}
...
{
  if (!freeAttack && IsDone(freeAttack)) return FALSE;
...
BOOL COMBATANT::CanAddTarget(int target)
{
  ...
  else
  {
    ...
    if (!m_pCharacter->targets.CanAddTarget(pTarget->GetNbrHD(), targrange))
    {
      return FALSE;
    }
    else
...
BOOL COMBATANT::AddTarget(COMBATANT &targ, int range)
{
  ...
  return (m_pCharacter->targets.AddTarget(targ.self, targ.GetNbrHD(), range, targX, targY));
}
...
//                               m_pCharacter->targets.Height());   // height
    ...
    case LingerAreaCone: // cone starts at x,y, extends in 'dir' direction
      ...
      GetCombatantsAndTilesInCone(targlist, maplist,
                          ...
                          mapx,mapy,            // target
                          //m_pCharacter->targets.m_MaxRange,   // length
                          m_pCharacter->targets.Height(),   // length
                          //m_pCharacter->targets.m_MaxTargets  // width at end of cone
                          m_pCharacter->targets.Width()  // width at end of cone
                         );      
      //GetMapTilesInCone(maplist, 
      //                  mapx,mapy,            // start
      //                  targets.m_MaxRange,   // length
...
BOOL COMBATANT::IsAttackPossible(void)
{
  ...
  {       
    ...
    pWeapon = itemData.GetItem(wpn_ID);
    {
      ...
      result = pWeapon->specAbs.RunScripts(CAN_AIM, ScriptCallback_RunAllScripts, NULL, GetName(), "");
      if (result.IsEmpty())
      {
        result = m_pCharacter->specAbs.RunScripts(CAN_AIM, ScriptCallback_RunAllScripts, NULL, GetName(), "");
      };
    };
...
BOOL COMBATANT::MoveCombatant(int newX, int newY, BOOL allowZeroMoveAttack)
{
  ...
  if (   ( (dude != NO_DUDE) && (allowZeroMoveAttack) )
      || ( m_iMovement < GetAdjMaxMovement()-(movePoints-1) )
     )
  {     
    if (coordsOnMap(newX, newY, width, height))
    {     
...

          {
                                         ...
                                         false))
            {
              ...
              WriteDebugString("TFA - Call script 'Guarding-CanGuardAttack(%s,%s,%s)'\n",
                                ...
                                hookParameters[7]);
#endif
              result = tempCOMBATANT->combatantSA.RunScripts(
...
void COMBATANT::InitFromMonsterData(const MONSTER_DATA *pMonster, BOOL IsFriendly, const ITEM_LIST &items, const MONEY_SACK &msack)
{
...

  {
    ...
    pMonster->specAbs.RunScripts(CREATE_MONSTER_COMBATANT, ScriptCallback_RunAllScripts, NULL, pMonster->Name, "");
...
void COMBATANT::OnStartCombatantAction()
{
  ...
  ClearHookParameters();
  */
  combatantSA.RunScripts(ON_START_COMBATANT_ACTION, ScriptCallback_RunAllScripts, NULL, "OnStartCombatantAction",m_pCharacter->GetName());
  ClearCharContext();
...
#ifdef newCombatant
    combatantSA.RunScripts(AUTO_ACTION, ScriptCallback_RunAllScripts, NULL, "OnAuto", GetName());
#else
...
BOOL COMBATANT::ModifyAttackThac0(const CHARACTER *pTarget,int *pVal) const
{
  ...
  else if (pTarget->HasGnomeTHAC0Penalty())
  {
    if (raceData.HasGnomeResistance(race()))
    {
      (*pVal) -= 1; // +1 bonus to THAC0
...
CString COMBATANT::RunPSScripts(const CString& scriptName)
{
  CString result;
  SPELL_DATA *pSpell;
