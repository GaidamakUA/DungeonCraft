...
bool GameEvent::OnTestTrigger(bool allowForwardStep)
{
  ...
  if (result)
  {
    if (allowForwardStep)
    {
      //if (virtualGameTime >= 0x12aff2)
      //{
      //  WriteDebugString("RunEvent.cpp line 385\n");
      //  ASSERT(1);
      //};
...
void SPLASH_DATA::OnDraw(void)
{
  ...
  if ((hImage >= 0)/* && (bActive)*/)
  {
    ...
    WriteDebugString("SPLASH_DATA::OnDraw\n");
    DisplayTextAndFlip();
    ...
    if (IsFirstScreen)
    {
      ...
      GraphicsMgr.DrawFont(VersionText_x,VersionText_y, GameVersionInfo, globalData.WhiteFont, FALSE, FALSE, FALSE);
    }
  }
}
...
*/ //NotImplemented(0xbe54a8, false); PickSpells = false;
...
/*      
 ...
 NotImplemented(0xbe41bb, false); */ knowSpellsAtCreation = TRUE;
      ...
      else // impassable obstical in our path
      {
        ...
        if (isLocked || isSpelled || isSpecialKeyed)
        {
          ...
          // Does anyone know the proper spell?
          {
            int i, n;
            SPELL_ID spellID;
            spellID = levelData.GetUnlockSpell(party.Posx, party.Posy, party.facing);
            if (!spellID.IsEmpty())
            {
              ...
              for (i=0; i<n; i++)
              {
                ...
                if (pCharacter->HaveSpell(spellID, true))
                {
                  caster = true;
                  m_openSpellID = spellID;
                ...
                for (k=0; (k<num)&&(!caster); k++)
                {
                  ...
                  for (j=0; j<m; j++)
                  {
                    ...
                    if (pCharSpell->IsMemorized())
                    {
                      ...
                      result = pSpell->RunSpellScripts(spellID, ScriptCallback_RunAllScripts, NULL);
...
void ADVENTURE_MENU_DATA::OnReturnToTopOfQueue()
{
  ...
  switch (myState) 
  {
    case TASK_MovePreMove0: reldir = 0; myState = TASK_MovePostMove0; break;
  ...
  case TASK_MovePostMove7:
    {
      ...
      QueueMapEvents(false); // false because we already did the events here.
...
void ADVENTURE_MENU_DATA::QueueMapEvents(bool execute_XY_events)
{
  ...
  case TASKMSG_AllowForwardStep:
    {
      //if (virtualGameTime >= 0x12aff2)
      //{
      //  WriteDebugString("RunEvent.cpp line 5221\n");
      //  ASSERT(1);
      //};
...
void SCRIPT_CHOICE_DATA::OnInitialEvent(void)
{
  ...
  //party.tradeQty = 0;
  //setTreasureEventState(m_pCallerEvent);
  menu.setMenu(ScriptChoiceMenuData);
  menu.setHorzOrient();
  menu.MapKeyCodeToMenuItem(KC_ESCAPE, 6);
  //SetMyState(TASK_TakeTreasure);
...
#endif
    choices = pCombatant->RunCombatantScripts(ON_READYCOMBATEQUIPMENT, ScriptCallback_RunAllScripts, NULL);
    ClearCharContext();
    menu.setCurrentItem(1);
    if (choices.IsEmpty())
    {
  ...
  case 1: // select
    {
      ...
      scriptContext.SetCharacterContext(pCombatant);
#endif
      pCombatant->RunCombatantScripts(ON_READYCOMBATEQUIPMENT, ScriptCallback_RunAllScripts, NULL);
#ifdef newCombatant
      pCombatant->m_pCharacter->RunCharacterScripts(ON_READYCOMBATEQUIPMENT, ScriptCallback_RunAllScripts, NULL);
#else
...
void ITEMS_MENU_DATA::OnKeypress(key_code key, char ascii)
{
  ...
  case 9: // "ID ITEM"
    if (m_bCallIdentifyHooks)
    {
      ...
      result = pDude->RunCharacterScripts(IDENTIFY, ScriptCallback_LookForChar, "Y");
      if (!result.IsEmpty())
      {
        int i = charItemListText.GetCharIndex(itemTextIndex);
...
#define NEW_EXAMINE
        ...
        if (menu.isItemActive(10))
        {
          ...
          //GLOBAL_ITEM_ID giID;
            ...
            pDude->RunCharacterScripts(CAN_EXAMINE_OR_WHATEVER, ScriptCallback_RunAllScripts, NULL);
            result = pItem->RunItemScripts(CAN_EXAMINE_OR_WHATEVER, ScriptCallback_RunAllScripts, NULL);
            if (!result.IsEmpty() && (result[0] == 'Y'))
            {
  ...
  if (m_bCallIdentifyHooks)
  {
    ...
    scriptContext.SetCharacterContext(pDude);
    hookParameters[4].Format("%d", i);
    result = pDude->RunCharacterScripts(CAN_IDENTIFY, ScriptCallback_LookForChar, "Y");
    m_saveParameter5 = hookParameters[5];
    if (result.IsEmpty())
    {
      m_bCallIdentifyHooks = FALSE;
  ...
  case 10: // "ZAP"
    {
      ...
      pTextEvent=new TEXT_EVENT_DATA;
      pTextEvent->text="**SHAZAM**ZapCmd()";
      //textEvent->pic = pic;
      //textEvent->HighlightText=FALSE;
...
//    if (virtualGameTime >= 0x12aff2)
//    {
//        WriteDebugString("RunEvent.cpp line 9046\n");
//        ASSERT(1);
//    };
  }
  return true;
...
*/
  ...
  if (virtualGameTime >= 0x12aff2)
  {
    WriteDebugString("RunEvent line 14400 menu item = %d\n", menu.currentItem());
    ASSERT(1);
  };
  switch (menu.currentItem()) 
  {
...
void COMBAT_EVENT_DATA::ChangeToTaskState(TASKSTATE state)
{
  ...
  case TASK_CombatStartBgMusic:
    ...
    break;
  case TASK_CombatMainMenu:
    //if (virtualGameTime >= 0x1293f0)
    //{
    //  ASSERT(1);
    //  WriteDebugString("RunEvent.cpp line 14496\n");
    //};
    ...
    Invalidate();
    {
      ...
      if (curr != NO_DUDE)
      {
        ...
        dude->GetContext(&actor);
        SetCharContext(&actor);
        dude->RunCombatantScripts(COMBAT_MAIN_MENU, ScriptCallback_RunAllScripts, NULL);
  ...
  case TASK_CombatAttacking:
    ...
    if (virtualGameTime == 0x12a3e)
    {
      int kkk=1;
      //WriteDebugString("DEBUG - Return; TASK_CombatAttacking\n");
    };
...
void COMBAT_EVENT_DATA::CheckForSpellLingeringScripts(void)
{
  ...
  pSdata = spellData.GetSpell(spellID);
  {
    ...
    scriptName = pSdata->RunSpellScripts(SPELL_LINGERING_SCRIPTS,
                                         ScriptCallback_RunAllScripts, 
                                         NULL);
...
//  };
  ...
  if (virtualGameTime >= 46170)
  {
  //  WriteDebugString("RunEvent.cpp line 15441\n");
    int kkk=1;  // state = TASK_CombatDelay_DisplayAttackMsg; TASK_CombatAttacking
  };
  state = GetMyState();
...
//        WriteDebugString("DEBUG - Return to line 15106\n");
        ...
        case OCS_Attacking: // auto
          /* 20111022 PRS
             Free Attacks and Guarding Attacks are Mele attacks.
  ...
  case TASK_CombatSpellMissileAnimate: 
    {
      ...
      if (dude->m_pCharacter->targets.NumTargets() > 0)
      {
        ...
        SetTargetContext(&actor);
        pSdata->RunSpellScripts(REDIRECT_SPELL, ScriptCallback_RunAllScripts, NULL);
        pTarget->m_pCharacter->RunCharacterScripts(REDIRECT_SPELL, ScriptCallback_RunAllScripts, NULL);
        ClearTargetContext();
  ...
  case TASK_CombatRedirectedSpellFinalStage:
    { 
      ...
      if (dude!=NULL)
      {
        ...
        {
          NotImplemented(0x5321aba,false);
          //COMBATANT *dude = combatData.getActiveCastingCharacter();
          //COMBATANT *pTarget;
          //if (dude!=NULL)
          //{
  ...
  case TASK_CombatDelay_NewCombatant: // Should be the first time we see this fellow.
    {
      ...
      //{
      //  NotImplemented(0x16ac, false);
      //};
//      WriteDebugString("DEBUG - Return TASK_CombatDelay_NewCombatant\n");
      if (virtualGameTime == 0x8250) // Slug was guarding when Troll moved up
      {
        int kkk=1;
      };
      if (virtualGameTime == 0x1293f0) // 3 moved against guarding 1, now it is time for 3 to move again.
      {
        int kkk=1;
      };
...
//        WriteDebugString("DEBUG - OCS = %d\n", OCS);
        ...
        case OCS_NewCombatant: //added 20111114
          ...
          return;
        case OCS_Guarding:
          if (virtualGameTime == 0xf4ec)
          {
            int kkk=1;
          };
#ifdef D20140707
//          WriteDebugString("DEBUG - Changing to CombatAttacking\n");
          ChangeToTaskState(TASK_CombatAttacking);
//          ChangeToTaskState(TASK_StartFreeAttack);
#else
          ChangeToTaskState(TASK_CombatGuardMenu);
#endif
          return;
        ...
        case OCS_Moving: // auto
          {
            ...
            if (updateScreen != 0) 
            {
              ...
              OCS = combatData.GetCombatState();
              if (virtualGameTime == 0x6324)
              {
                int kkk=1; // 3 moved against guarding 1;
              };
              switch (OCS)
              {
              //case OCS_None:
      ...
      else
      {
        ...
        switch(OCS)
        {
          ...
          case OCS_None:
            ...
            //if (virtualGameTime == 0x1293f0)
            //{
            //  WriteDebugString("RunEvent.cpp line 18076\n");
            //  ASSERT(1);
            //};
            ChangeToTaskState(TASK_CombatMainMenu);
            break;
...
bool COMBAT_EVENT_DATA::OnIdle(void)
{
  ...
  //{
  //  ASSERT(1);
  //  WriteDebugString("RunEvent.cpp line 18258\n");
  //};
  OCS = combatData.GetCombatState();
...
bool COMBAT_EVENT_DATA::OnCycle(void)
{
  ...
  //{
  //  ASSERT(1);
  //  WriteDebugString("RunEvent.cpp line 18424\n");
  //};
  //PRS July 2009
  //
...
void COMBAT_EVENT_DATA::OnLeaveQueue(void)
{
  ...
  //{
  //  ASSERT(1);
  //  WriteDebugString("RunEvent.cpp line 18469\n");
  //};
...
unsigned int COMBAT_EVENT_DATA::OnTaskMessage(TASKMESSAGE msg, TASKSTATE taskState)
{
  ...
  //{
  //  ASSERT(1);
  //  WriteDebugString("RunEvent.cpp line 18571\n");
  //};
  switch (msg)
  {
...
void COMBAT_AIM_MANUAL_MENU_DATA::OnInitialEvent(void)
{
  ...
  combatData.RemoveCurrTarget();
  combatData.ResetAim();
  menu.setMenu(AimManualMenuData);
  menu.setHorzOrient();
    ...
    {    
      ...
      if (target != NO_DUDE)
      {
        ...
        if (m_pCaster->m_pCharacter->targets.m_SelectingUnits)
        {
          ...
          if (!m_pCaster->AddTarget(*pTarget, targrange))
        ...
        else
        {
          ...
          int dirX, dirY;
          dirX = pTarget->GetCenterX() - m_pCaster->GetCenterX();
          dirY = pTarget->GetCenterY() - m_pCaster->GetCenterY();
          if (!m_pCaster->AddMapTarget(pTarget->GetCenterX(), pTarget->GetCenterY(), dir, dirX, dirY))
            WriteDebugString("Failed AddTarget() in COMBAT_SPELL_AIM_MENU_DATA\n");
        }
...
void COMBAT_SPELL_AIM_MANUAL_MENU_DATA::OnKeypress(key_code key, char ascii)
{
  ...
  };
  {
    ...
    result = m_pSpell->RunSpellScripts(IS_VALID_TARGET, ScriptCallback_LookForChar, "YN");
    if (result == "")
    {
      if (pTarget != NULL)
      {
        result = pTarget->RunCombatantScripts(IS_VALID_TARGET,  ScriptCallback_LookForChar, "YN");
      };
    };
    m_isValidTarget = ' ';
    if (!result.IsEmpty())
...
void TIME_DELAY_MSG_DATA::OnInitialEvent(void)
{
  int attackResult;
  int dude;
  //if (virtualGameTime >= 0x12aff2) // 1 is guarding when 3 approaches
  //{
  //  WriteDebugString("RunEvent.cpp line 20366\n");
  //  int kkk=1; // 
  //};
  m_iDeathIndex = -1;
  m_timerrcvd=false;
  ...
  default:
    {
      //if (virtualGameTime == 0x12aff2)
      //{
      //  WriteDebugString("RunEvent.cpp line 20405\n");
      //  int kkk=1;
      //};
      m_iDeathIndex = -1;
  ...
  //{
  //  ASSERT(1);
  //  WriteDebugString("RunEvent.cpp line 20412 state = %d\n", m_state);
  //};
  switch (m_state)
  {
  ...
  //if (virtualGameTime >= 0x12aff2)
  //{
  //  WriteDebugString("RunEvent.cpp line 20464 delay=%d\n", m_delay);
  //  ASSERT(1);
  //};
  SetEventTimer(TASKTIMER_TimeDelayMsg, m_delay);
}
...
bool TIME_DELAY_MSG_DATA::OnTimer(UINT TimerId)
{
   if (TimerId == TASKTIMER_TimeDelayMsg)
   {
    ...
    {   
      ...
      default:
        {
          //if (virtualGameTime == 0x129198)
          //{
          //  WriteDebugString("RunEvent.cpp line 20469\n");
          //  int kkk=1;
          //};
          m_timerrcvd=true;
...
void COMBAT_ROUND_DELAY_DATA::OnInitialEvent(void)
{
  ...
  //if (virtualGameTime == 0x129198)
  //{
  //  WriteDebugString("RunEvent.cpp line 20553\n");
  //  ASSERT(1);
  //};
...
void COMBAT_SPECIAL_ACTION_DATA::OnReturnToTopOfQueue(void)
{
  ...
  SetTargetContext (&actor);
  pAttacker->RunCombatantScripts(COMBAT_SPECIAL_ACTION, ScriptCallback_RunAllScripts, NULL);
  ClearTargetContext();
  combatData.CheckForDeadGuys();
...
void INITIAL_MU_SPELLS_MENU_DATA::AutoScribe(void)
{
  ...
  for (i=0; i<n; i++)
  {
    ...
    if (!m_dude.KnowSpell(pSpell, true))        
    {
      CString msg;
...
int INITIAL_MU_SPELLS_MENU_DATA::ListAllSpells(void)
  ...
  for (int i=0; i<n; i++)
  {
    ...
    result = pClass->RunClassScripts(KNOWABLE_SPELLS, ScriptCallback_RunAllScripts, NULL);
    if (result.IsEmpty())
    {
      result = pSpell->RunSpellScripts(KNOWABLE_SPELLS, ScriptCallback_RunAllScripts, NULL);
    };
    if (result.IsEmpty())
    {
      probability = 100;
...
void INITIAL_MU_SPELLS_MENU_DATA::OnInitialEvent(void)
{
  ...
  ClearFormattedText(errorTextData);
  int maxSpellLevel;
  menu.setMenu(InitialSpellsMenuData);
  menu.setHorzOrient();
  //menu.MapKeyCodeToMenuItem(KC_ESCAPE, 4);
  spellListText.Clear();
...
int LEARN_SPELLS_MENU::ListAllSpells(void)
{
  ...
  while (spellList.GetLength() > 1)
  {
    ...
    };
    {
      ...
      result = pClass->RunClassScripts(LEARN_SPELLS, ScriptCallback_RunAllScripts, NULL);
      if (result.IsEmpty())
      {
        result = pSpell->RunSpellScripts(LEARN_SPELLS, ScriptCallback_RunAllScripts, NULL);
      };
      if (result.IsEmpty())
      {
        probability = 100;
...
void LEARN_SPELLS_MENU::OnInitialEvent(void)
{
  ...
  menu.setMenu(LearnSpellsMenuData);
  menu.setHorzOrient();
  //menu.MapKeyCodeToMenuItem(KC_ESCAPE, 4);
  spellListText.Clear();
...
void LEVEL_UP_LEARN_SPELLS_MENU::OnInitialEvent(void)
{
  ClearFormattedText(textData);
  int maxSpellLevel, level;
  menu.setMenu(LevelUpLearnSpellsMenuData);
  menu.setHorzOrient();
  //menu.MapKeyCodeToMenuItem(KC_ESCAPE, 4);
  spellListText.Clear();
  maxSpellLevel = ListAllSpells();
...
void CHOOSE_FROM_LIST_MENU_DATA::OnKeypress(key_code key, char ascii)
{
  ...
  */
  ...
  case 1: // "SELECT"
    {
      this->m_result = lineListText.GetLineLine(currLineTextIndex());
      PopEvent();
    };
    break;    
  case 2: // "NEXT"
...
void CHOOSE_FROM_LIST_MENU_DATA::OnInitialEvent(void)
{
  CString choice;
  ClearFormattedText(textData);
  menu.setMenu(SelectFromListMenuData);
  menu.setHorzOrient();
  menu.MapKeyCodeToMenuItem(KC_ESCAPE, 4);
        ...
        {
          {
            ...
            result = pSpell->RunSpellScripts(OVERRIDE_SPELL_SELECTION,ScriptCallback_RunAllScripts, NULL);
            if (!result.IsEmpty())
            {
              m_pChooseSpell = new CHOOSE_FROM_LIST_MENU_DATA;
              m_pChooseSpell->m_list = result;
              PushEvent(m_pChooseSpell, KeepEvent);
              break;
            };
          };
...
{
  if (m_pChooseSpell != NULL)
  {
    CString spellName;
    //GLOBAL_SPELL_ID gsID;
    SPELL_ID spellID;
    spellName = this->m_pChooseSpell->m_result;
    delete m_pChooseSpell;
    m_pChooseSpell = NULL;
  ...
  int casterLevel = -1;
  {
    ...
    default:
      ...
      { 
        ...
        result = pSpData->RunSpellScripts(SPELL_NUMTARGETS, ScriptCallback_RunAllScripts, NULL);
        if (result.GetLength()> 0)
        {
    ...
    case SelectedByCount:
      { 
                                     ...
                                     &casterLevel); 
        {
          ...
          scriptContext.SetAttackerContext(m_pCaster);
          hookParameters[5].Format("%d", int(val));
          result = pSpData->RunSpellScripts(SPELL_RANGE, ScriptCallback_RunAllScripts, NULL);
          if (result.GetLength()> 0)
          {
            sscanf(result,"%lf",&val);
          };
  ...
  {        
    ...
    else
    {
      ...
      for (int i=0; i<combatData.NumCombatants(); i++)
      {
        ...
        {
          if (!m_pCaster->AddTarget(*combatData.GetCombatant(i)))
...
*/ NotImplemented(0x4d67ab9, false);
  ...
  case 3: // scribe
        ...
        || (m_pOrigEvent->GetEventType() != Combat))
    {
      ...
      pCharacter = &GetActiveChar(m_pOrigEvent);
      {
        ...
        scriptContext.SetCharacterContext(pCharacter);
        scriptContext.SetClassContext(pClass);
        pCharacter->RunCharacterScripts(SCRIBE_OR_WHATEVER, ScriptCallback_RunAllScripts, "");
        pClass->RunClassScripts(SCRIBE_OR_WHATEVER, ScriptCallback_RunAllScripts, "");
        ClearCharContext();
...
void MAGIC_MENU_DATA::OnUpdateUI(void)
{
      ...
      || (m_pOrigEvent->GetEventType() != Combat))
  {
    ...
    if (pCharacter != m_pActiveCharacter) // Avoids excessive script activations
    {
      ...
      scriptContext.SetCharacterContext(pCharacter);
      scriptContext.SetClassContext(pClass);
      pCharacter->RunCharacterScripts(CAN_SCRIBE_OR_WHATEVER, ScriptCallback_RunAllScripts, "");
      pClass->RunClassScripts(CAN_SCRIBE_OR_WHATEVER, ScriptCallback_RunAllScripts, "");
      ClearCharContext();
