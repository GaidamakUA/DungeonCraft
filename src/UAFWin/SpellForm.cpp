   ...
   {      
     int EnumOffset = (displaycount*TEXT_FORM::repeatIncr);
...

      {
        ...
        if (spellTextList.UseSpellEffectList())
        {
          tmpRECT = pForm->SetText(STSF_name+EnumOffset, spellTextList.GetSpellEffectText(currItem), color);
        }
        else if (spellTextList.UseMemorizeLayout())
        {
          tmpRECT = pForm->SetText(STSF_name+EnumOffset, spellTextList.GetSpellNameText(currItem), color);
        }
        else
        {
          tmpRECT = pForm->SetText(STSF_name+EnumOffset, spellTextList.GetSpellNameText(currItem), color);
