 ...
 *       NotImplemented(0xc98f4a,FALSE);//temp.specAbs.EnableAllFor(&temp);
 *
 ...
 */
   ...
   // the combatant will find him actually a part of the combat.                           //*
      temp.Clear();                                                                        //*
      //NotImplemented(0xc54d30,FALSE); // I cannot see how this ever worked.              //*
                                      // Either it encountered a NotImplemented()          //*
 ...
 *   int PreSpecAbCount = temp.spellEffects.GetCount();
 *   // turn on the special abilities
 *   //NotImplemented(0xc98f7c,FALSE);//temp.specAbs.EnableAllFor(&temp);
 *
 *   int PostSpecAbCount = temp.spellEffects.GetCount();
 *   int Diff = PostSpecAbCount - PreSpecAbCount;
 *   if (Diff > 0)
...
{   
   ...
   while ((dude = QComb.Top()) != NO_DUDE)
   {
     ...
     if (m_aCombatants[dude].IsDone(false))
     {
       QComb.Pop();
     }
     else
   ...
   while ((m_iCurrInitiative <= 22) && (!found))
   {
      ...
      while ((i < m_iNumCombatants) && (!found))
      {
        if (m_aCombatants[i].m_iInitiative == m_iCurrInitiative) 
        {
          ...
          if  (!m_aCombatants[i].IsDone(false))
          {
            dude = i;
            found = TRUE;
          };
...
bool COMBAT_DATA::CanMoveCursor(const COMBATANT *pCaster, int x, int y)
{
  ...
  if (d2 > pChar->targets.MaxRange() * pChar->targets.MaxRange()) return false;
...
// PRS20110223  m_iPrevRndCombatant = prevDude;
  ...
  if (    (dude == NO_DUDE) 
       || (m_aCombatants[dude].IsDone(QComb.NumFreeAttacks()+QComb.NumGuardAttacks() > 0))
     )
  {
    getNextCombatant();
...
void COMBAT_DATA::StartNewRound()
{
  ...
  for (i=0; i<m_iNumCombatants; i++)
  {
    ...
    if ((m_aCombatants[i].charCanTakeAction()) && (m_aCombatants[i].IsDone(false)))
    {      
      ...
      { 
        ...
        for (j=0; j<n; j++)
        {
          if (pAURA->m_combatants[j] == i)
          {
            ...
            pAURA->specialAbilities.RunScripts(AURA_EFFECT, ScriptCallback_RunAllScripts, NULL, "", "");
            PopAuraReference();
            break;
          };
        };
...

  {
    ...
    for (pos=m_auras.GetHeadPosition(); pos!=NULL;)
    {
      ...
      PushAuraReference(pAURA->auraID);
      m_auras.GetNext(pos);
      pAURA->specialAbilities.RunScripts(AURA_ROUND, ScriptCallback_RunAllScripts, NULL, "", "");
      PopAuraReference();
    };
  };
  m_iCurrInitiative = 1;
...
{ 
  ...
  int moves = pCombatant->GetAdjMaxMovement() - pCombatant->m_iMovement;
...
{ 
  ...
  if (curr != NO_DUDE)
  {
    if (!turningSummary.IsValid())
    {
      ...
      result = dude->RunCombatantScripts(CAN_TURN, ScriptCallback_RunAllScripts, NULL);
...
void COMBAT_DATA::TurnUndead() 
{
  ...
  //Get a list of the undead types that are to be turned.
  {
    ...
    //pClass = &classData[dude->GetClass()];
    scriptResult = dude->RunCombatantScripts(TURN_ATTEMPT, ScriptCallback_RunAllScripts, NULL);
    col = 0;
...
{ 
  ...
  if ( (curr != NO_DUDE) && (currTarg != NO_DUDE))
    return m_aCombatants[curr].StartAttack(currTarg, QComb.NumFreeAttacks() + QComb.NumGuardAttacks()); 
  return FALSE;
}
  ...
  {            
    ...
    //val = pCaster->EvalDiceAsClass(pSdata->Duration, pSdata->schoolMask, NULL);
    {
      ...
      { 
        ...
        result = pSdata->RunSpellScripts(SPELL_CASTER_LEVEL, ScriptCallback_RunAllScripts, NULL);
...
//
//*****************************************************************************
void COMBAT_DATA::RemoveSpellEffect(DWORD parent, SPELL_DATA *pSpell, bool endSpell)
{
  for (int i=0; i<m_iNumCombatants; i++)
      ...
      cellIndex = y*MAX_TERRAIN_WIDTH+x;
      {
        if (pAURA->cells[cellIndex] & 1)
        {
          ...
          if (j == pAURA->m_combatants.GetSize())
          {
            ...
            SCRIPT_CONTEXT scriptContext;
            scriptContext.SetCombatantContext(&m_aCombatants[i]);
            pAURA->specialAbilities.RunScripts(AURA_ENTER, ScriptCallback_RunAllScripts, NULL, "", "");
        ...
        else
        {
          ...
          if (j < pAURA->m_combatants.GetSize())
          {
            ...
            SCRIPT_CONTEXT scriptContext;
            scriptContext.SetCombatantContext(&m_aCombatants[i]);
            pAURA->specialAbilities.RunScripts(AURA_EXIT, ScriptCallback_RunAllScripts, NULL, "", "");
                            ...
                            const CString& s5)
{
  ...
  };
  {
    ...
    pAURA->specialAbilities.RunScripts(AURA_CREATE, ScriptCallback_RunAllScripts, NULL, "", "");
