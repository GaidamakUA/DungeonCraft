...
static DISPLAY_FORM statsForm[] =
{
  ...
  STF(EXP      ,none       ,LX    ,class  ,  20)  // 1st baseclass level/experience
...
{ 
   ...
   CLASS_ID charTrueClass = dude.GetClass();
   {
     ...
     if (charCurrClass!=charTrueClass)
       pForm->SetText( STF_class, className, AdjColor);
     else
       pForm->SetText( STF_class, className);
   };
...
CString GrMoveTo(const CString& name)
{
  ...
  if (pGD != NULL)
  {
    gc.cursorX = gc.anchorX = pGD->x;
    gc.cursorY = gc.anchorY = pGD->y;
  };
  return "";
}
...
CString GrMove(const CString& name)
{
  ...
  if (pGD != NULL)
  {
    gc.anchorX += pGD->x;
    gc.anchorY += pGD->y;
    gc.cursorX = gc.anchorX;
    gc.cursorY = gc.anchorY;
  };
  return "";
}
...
CString GrPrint(const CString& text)
{
  displayText( gc.cursorX, gc.cursorY, text, gc.color, false, false, true);
  gc.cursorX += GraphicsMgr.GetTextWidth(text, text.GetLength(), false);
  return "";
}

CString GrPrtLF(const CString& text)
{
  displayText(gc.cursorX, gc.cursorY, text, gc.color, false, false, true);
  gc.anchorX += gc.linefeedX;
  gc.anchorY += gc.linefeedY;
  gc.cursorX = gc.anchorX;
  gc.cursorY = gc.anchorY;
  return "";
}
