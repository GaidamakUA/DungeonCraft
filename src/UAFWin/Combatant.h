...
struct COMBATANT //: public CHARACTER
{
  ...
  inline BYTE GetAdjMaxMovement       (DWORD flags=DEFAULT_SPELL_EFFECT_FLAGS) const {return m_pCharacter->GetAdjMaxMovement(flags);}; 
...
/*  Questionable */
  ...
  void InstantSpellActivate(const SPELL_ID& spellID, 
                            int target, 
  ...
  CString RunPSScripts(const CString& scriptName);
